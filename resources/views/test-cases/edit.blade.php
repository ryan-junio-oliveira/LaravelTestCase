...
@section('title', 'Editar Caso de Teste - Sistema de Gerenciamento de Testes')
@section('page-title', 'Editar Caso de Teste')

@section('content')
<div class="max-w-3xl mx-auto">
    <div class="card">
        <div class="p-6 border-b">
            <h3 class="text-lg font-semibold text-gray-900">Editar Caso de Teste</h3>
        </div>

        <form action="{{ route('test-cases.update', $testCase) }}" method="POST" class="p-6 space-y-4">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <select id="status" 
                    ...
                    <option value="archived" {{ old('status', $testCase->status) === 'archived' ? 'selected' : '' }}>Arquivado</option>
                </select>
                @error('status')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
                        ...
                        <div class="step-item flex flex-col space-y-2 border p-4 rounded-lg">
                            <div>
                                ...
                                <textarea name="steps[{{ $index }}][action]" 
                                          rows="2"
                                          class="form-textarea @error('steps.'.$index.'.action') border-red-500 @enderror" 
                            ...
                            <div>
                                ...
                                <textarea name="steps[{{ $index }}][expected_result]" 
                                          rows="2"
                                          class="form-textarea @error('steps.'.$index.'.expected_result') border-red-500 @enderror" 
