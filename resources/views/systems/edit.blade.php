...
@section('title', 'Editar Sistema - Sistema de Gerenciamento de Testes')
@section('page-title', 'Editar Sistema')

@section('content')
<div class="max-w-2xl mx-auto">
    <div class="card">
        <div class="p-6 border-b">
            <h3 class="text-lg font-semibold text-gray-900">Editar Sistema</h3>
        </div>

        <form action="{{ route('systems.update', $system) }}" method="POST" class="p-6 space-y-4">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <select id="status" 
                    ...
                    <option value="active" {{ old('status', $system->status) === 'active' ? 'selected' : '' }}>Ativo</option>
                    <option value="inactive" {{ old('status', $system->status) === 'inactive' ? 'selected' : '' }}>Inativo</option>
                    <option value="maintenance" {{ old('status', $system->status) === 'maintenance' ? 'selected' : '' }}>Manutenção</option>
                </select>
                @error('status')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
